...
if [[ "${CURRENT_BRANCH}" == "master" ]]; then
  ...
  if [[ "${#CURRENT_TAGS[@]}" -gt "0" ]]; then
    ...
    # Artifacts should have been collected to CI server by this point, so now we can just copy the packages that should be pushed to a temp directory, and push all at once with one command
    PUSH_DIR=$(mktemp -d)
