...
namespace UtilPack.AsyncEnumeration.LINQ
{
   internal class SelectEnumerator<T, U> : IAsyncEnumerator<U>
   {
      ...
      {
         var item = _source.TryGetNext( out success );
...
public static partial class E_UtilPack
{
   ...
   public static IAsyncEnumerable<U> Select<T, U>( this IAsyncEnumerable<T> enumerable, Func<T, U> selector )
   {
      ...
      return new EnumerableWrapper<U>( () => enumerable.GetAsyncEnumerator().Select( selector ) );
   }
   ...
   public static IAsyncEnumerable<U> Select<T, U>( this IAsyncEnumerable<T> enumerable, Func<T, ValueTask<U>> asyncSelector )
   {
      ...
      return new EnumerableWrapper<U>( () => enumerable.GetAsyncEnumerator().Select( asyncSelector ) );
   }

   /// <summary>
   /// This extension method will return <see cref="IAsyncEnumerator{T}"/> which will return items as transformed by given selector callback.
   /// </summary>
   /// <typeparam name="T">The type of source items.</typeparam>
   /// <typeparam name="U">The type of target items.</typeparam>
   ...
   public static IAsyncEnumerator<U> Select<T, U>( this IAsyncEnumerator<T> enumerator, Func<T, U> selector )
   {
      ...
      ArgumentValidator.ValidateNotNull( nameof( selector ), selector );
      return new SelectEnumerator<T, U>( enumerator, selector );
   }

   /// <summary>
   /// This extension method will return <see cref="IAsyncEnumerator{T}"/> which will return items as transformed by given asynchronous selector callback.
   /// </summary>
   /// <typeparam name="T">The type of source items.</typeparam>
   /// <typeparam name="U">The type of target items.</typeparam>
   /// <param name="enumerator">This <see cref="IAsyncEnumerator{T}"/>.</param>
   ...
   public static IAsyncEnumerator<U> Select<T, U>( this IAsyncEnumerator<T> enumerator, Func<T, ValueTask<U>> asyncSelector )
   {
      ArgumentValidator.ValidateNotNullReference( enumerator );
      ArgumentValidator.ValidateNotNull( nameof( asyncSelector ), asyncSelector );
      return new AsyncSelectEnumerator<T, U>( enumerator, asyncSelector );
   }
}